...
const targetDir = join(tmpdir(), 'weft-price-updater-worker')
const targetWorkerdPath = join(targetDir, 'workerd')
const userArgs = process.argv.slice(2)
const defaultVars = hasWranglerVar(userArgs, 'DRY_RUN') ? [] : ['--var', 'DRY_RUN:true']
...
function hasWranglerVar(args, name) {
  const inlinePrefix = `--var=${name}:`
  const valuePrefix = `${name}:`

  return args.some((arg, index) =>
    arg.startsWith(inlinePrefix)
    || (arg === '--var' && args[index + 1]?.startsWith(valuePrefix)),
  )
}
